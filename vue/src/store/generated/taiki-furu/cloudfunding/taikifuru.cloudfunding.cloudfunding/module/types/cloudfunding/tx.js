...
const baseMsgStopProject = { creator: "", id: 0 };
export const MsgStopProject = {
    ...
    decode(input, length) {
        const reader = input instanceof Uint8Array ? new Reader(input) : input;
        let end = length === undefined ? reader.len : reader.pos + length;
        const message = { ...baseMsgStopProject };
        while (reader.pos < end) {
            const tag = reader.uint32();
            switch (tag >>> 3) {
                case 1:
...
const baseMsgStopProjectResponse = {};
export const MsgStopProjectResponse = {
    ...
    decode(input, length) {
        const reader = input instanceof Uint8Array ? new Reader(input) : input;
        let end = length === undefined ? reader.len : reader.pos + length;
        const message = { ...baseMsgStopProjectResponse };
        while (reader.pos < end) {
            const tag = reader.uint32();
            switch (tag >>> 3) {
                default:
...
export class MsgClientImpl {
    ...
    StopProject(request) {
        ...
        const promise = this.rpc.request("taikifuru.cloudfunding.cloudfunding.Msg", "StopProject", data);
        return promise.then((data) => MsgStopProjectResponse.decode(new Reader(data)));
    }
}
var globalThis = (() => {
